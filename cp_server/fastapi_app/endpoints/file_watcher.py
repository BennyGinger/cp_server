...
async def setup_file_watcher(request: Request, payload: PayLoad)-> dict:
    # Retrieve the file watcher manager from the app state
    watcher_manager: FileWatcherManager = request.app.state.file_watcher_manager
    
    # Start the watcher
    try:
        await watcher_manager.start_watcher(**payload)
...
async def stop_file_watcher(request: Request, directory: str)-> dict:
    # Retrieve the file watcher manager from the app state
    watcher_manager = request.app.state.file_watcher_manager
