...
def test_setup_file_watcher(client, payload):
    ...
    expected_message = f"File watcher setup for directory: {payload['directory']}"
...
def test_stop_file_watcher(client):
    payload = {"directory": "/tmp/watch"}
    response = client.post("/stop-file-watcher", json=payload)
    assert response.status_code == 200
